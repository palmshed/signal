...
import { getTrafficSources, getUtmCampaigns, getLandingPages } from "@/lib/analytics/aggregates";
...
export async function GET(request: Request) {
  ...
  const sources = getTrafficSources(sinceIso);
  const campaigns = getUtmCampaigns(sinceIso);
  const landingPages = getLandingPages(sinceIso);

  return NextResponse.json({ sources, campaigns, landingPages });
}
