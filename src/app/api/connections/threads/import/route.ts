...
export async function POST(request: Request) {
  ...
  let body: { posts: ThreadsPostToImport[] };
  try {
  ...
  for (const post of posts) {
    const existing = db
      .select()
      .from(schema.posts)
      .where(and(eq(schema.posts.platform, "threads"), eq(schema.posts.platformPostId, post.platformPostId)))
      .get();
