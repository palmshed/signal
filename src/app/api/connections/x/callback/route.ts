...
export async function GET(request: Request) {
  ...
  if (!payload || payload.platform !== "x" || payload.state !== state) {
    clearOAuthCookie();
    return NextResponse.redirect(new URL("/admin/settings?connection_error=invalid_state", request.url));
  }

  try {
    const tokens = await xClient.exchange(code, payload.verifier);
    const me = await xClient.me(tokens.accessToken);
    saveConnection({
      platform: "x",
      user: me,
      tokens,
    ...
    clearOAuthCookie();
    const message = err instanceof Error ? err.message : "Failed to complete X connection.";
    return NextResponse.redirect(
  ...
  clearOAuthCookie();
  return NextResponse.redirect(new URL("/admin/settings?connection_ok=x", request.url));
}
